...
from ...application.service.stats_monthly_sales_user_sales_service import StatsMonthlySalesUserSalesService
...
class StatsMonthlySalesUserSalesApiController(APIView):
    """
    # CLASS : StatsMonthlySalesUserSalesApiController
    # AUTHOR : jung-gyuho
    # TIME : 2023/07/31 6:47 PM
    # DESCRIPTION
        - StatsMonthlySalesUserSalesApiController
        - MonthlySalesUserSales API
        - CRM 디렉터리 : stats
        - CRM 서비스 설명 :
        - CRM 서비스 호출 url :
    ...
    @swagger_auto_schema(tags=['CRM SYSTEM - Stats API'], operation_summary="CRM Stats MonthlySalesUserSales API",
                         operation_description="# DESIGNER SEVER에서 CRM SYSTEM으로 MonthlySalesUserSales 요청 API\n"
                                               "\n"
    ...
    def post(self, request, *args, **kwargs):
        ...
        # DESCRIPTION
            - API NAME : MonthlySalesUserSales API
            - API DESC : CRM MonthlySalesUserSales 진입경로
        ...
        service: StatsMonthlySalesUserSalesService = container.statsMonthlySalesUserSalesCrmServiceProvider()

        try:
            result = service.stats_monthly_sales_user_sales_crm(request.data,
