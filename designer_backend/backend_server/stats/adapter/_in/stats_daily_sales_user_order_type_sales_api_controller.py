...
from ...application.service.stats_daily_sales_user_order_type_sales_service import \
    StatsDailySalesUserOrderTypeSalesService
...
class StatsDailySalesUserOrderTypeSalesApiController(APIView):
    """
    # CLASS : StatsDailySalesUserOrderTypeSalesApiController
    # AUTHOR : jung-gyuho
    # TIME : 2023/08/04 2:34 PM
    # DESCRIPTION
        - StatsDailySalesUserOrderTypeSalesApiController
        - DailySalesUserOrderTypeSales API
        - CRM 디렉터리 : stats
        - CRM 서비스 설명 : 디자이너 매출조회 - 일, (타임아웃 20초 에러 발생, 데이터 많아서 조회 속도 느림, 처리 방안 확인 필요)
        - CRM 서비스 호출 url : /stats/getSalesUserOrdTypeSales/
    ...
    @swagger_auto_schema(tags=['CRM SYSTEM - Stats API'],
                         operation_summary="CRM Stats DailySalesUserOrderTypeSales API",
                         operation_description="# DESIGNER SEVER에서 CRM SYSTEM으로 DailySalesUserOrderTypeSales 요청 API\n"
                                               "\n"
    ...
    def post(self, request, *args, **kwargs):
        ...
        # DESCRIPTION
            - API NAME : DailySalesUserOrderTypeSales API
            - API DESC : CRM DailySalesUserOrderTypeSales 진입경로
            ...
            - REQUEST PARAMS :
                ...
                |searchStDt    |varchar|20        |TRUE|조회시작일   |20210801|
                |searchEdDt    |varchar|20        |TRUE|조회종료일   |20210831|
                |term    |varchar|20        |TRUE|월 / 일 => month / day   |day|
                |salesUserId    |varchar|20        |TRUE|디자이너아이디   |1214|
        ...
        service: StatsDailySalesUserOrderTypeSalesService = container.statsDailySalesUserOrderTypeSalesCrmServiceProvider()

        try:
            result = service.stats_daily_sales_user_order_type_sales_crm(request.data, accessToken=kwargs.get('accessToken', 'None'),
