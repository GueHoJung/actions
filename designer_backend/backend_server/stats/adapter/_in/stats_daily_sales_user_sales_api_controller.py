...
from ...application.service.stats_daily_sales_user_sales_service import StatsDailySalesUserSalesService
...
class StatsDailySalesUserSalesApiController(APIView):
    """
    # CLASS : StatsDailySalesUserSalesApiController
    # AUTHOR : jung-gyuho
    # TIME : 2023/07/31 5:12 PM
    # DESCRIPTION
        - StatsDailySalesUserSalesApiController
        - DailySalesUserSales API
        - CRM 디렉터리 : stats
        - CRM 서비스 설명 :
        - CRM 서비스 호출 url :
    ...
    @swagger_auto_schema(tags=['CRM SYSTEM - Stats API'], operation_summary="CRM Stats DailySalesUserSales API",
                         operation_description="# DESIGNER SEVER에서 CRM SYSTEM으로 DailySalesUserSales 요청 API\n"
                                               "\n"
    ...
    def post(self, request, *args, **kwargs):
        ...
        # DESCRIPTION
            - API NAME : DailySalesUserSales API
            - API DESC : CRM DailySalesUserSales 진입경로
        ...
        service: StatsDailySalesUserSalesService = container.statsDailySalesUserSalesCrmServiceProvider()

        try:
            result = service.stats_daily_sales_user_sales_crm(request.data, accessToken=kwargs.get('accessToken', 'None'),
