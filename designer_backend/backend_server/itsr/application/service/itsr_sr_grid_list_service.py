from ..port._in.itsr_sr_grid_list_in_port import ItsrSrGridListInPort
from ..port.out.itsr_sr_grid_list_out_port import ItsrSrGridListOutPort
...
class ItsrSrGridListService:
    ...
    def __init__(self, portInImpl: ItsrSrGridListInPort, portOutImpl: ItsrSrGridListOutPort):
        self.itsrIn = portInImpl
        self.itsrOut = portOutImpl

    def itsr_sr_grid_list_crm(self, *args, **kwargs):
        print(f"{self.__class__.__name__} itsr_sr_grid_list_crm *args ==> {args[0]}")

        data = self.itsrIn.itsr_in_port(self, args[0])

        for arg in args:
            print(f"{self.__class__.__name__} itsr_sr_grid_list_crm *args ==> {arg}")

        for kwarg in kwargs:
            print(f"{self.__class__.__name__} itsr_sr_grid_list_crm **kwargs ==> {kwarg}")

        API_HOST = getattr(settings, "CRM_HOST_IP", None)
        API_PORT = getattr(settings, "CRM_HOST_PORT", None)
        API_ADR = API_HOST + ":" + API_PORT
        print(f"Api host ==> {API_HOST}")
        result = self.itsrOut.itsr_out_port(self, API_ADR, "/itsr/getSrGridList/", "POST", data,
