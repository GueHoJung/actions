...
from ...application.service.customer_cust_taste_list_service import CustomerCustTasteListService
...
class CustomerCustTasteListApiController(APIView):
    """
    # CLASS : CustomerCustTasteListApiController
    # AUTHOR : jung-gyuho
    # TIME : 2023/07/28 10:07 PM
    # DESCRIPTION
        - CustomerCustTasteListApiController
        - CustTasteList API
        - CRM 디렉터리 : customer
        - CRM 서비스 호출 url : /customer/getCustTasteList/
    ...
    @swagger_auto_schema(tags=['CRM SYSTEM - Customer API'], operation_summary="CRM Customer CustTasteList API",
                         operation_description="# DESIGNER SEVER에서 CRM SYSTEM으로 CustTasteList 요청 API\n"
                                               "\n"
    ...
    def post(self, request, *args, **kwargs):
        ...
        # DESCRIPTION
            - API NAME : CustTasteList API
            - API DESC : CRM CustTasteList 진입경로
        ...
        service: CustomerCustTasteListService = container.customerCustTasteListCrmServiceProvider()

        try:
            result = service.customer_cust_taste_list_crm(request.data, accessToken=kwargs.get('accessToken', 'None'),
